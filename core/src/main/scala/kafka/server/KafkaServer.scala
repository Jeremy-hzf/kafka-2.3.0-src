...
class KafkaServer(val config: KafkaConfig, time: Time = Time.SYSTEM, threadNamePrefix: Option[String] = None,
  ...
  private val isShuttingDown = new AtomicBoolean(false)   //是否是关闭
  private val isStartingUp = new AtomicBoolean(false)     //是否是开启
  ...
  var adminManager: AdminManager = null       //管理员
  var tokenManager: DelegationTokenManager = null     //token管理器
  //动态配置处理器
